...
module.exports = (dbPoolInstance) => {
  ...
  let createNewCommList = (dataIn, callback) => {
    var creatorid = dataIn.userid;
    var listname = dataIn.listname;
    var listDescription = dataIn.description;
    
    
      let query = 'INSERT INTO commlistdetail (creatorid, listname, listDescription ) VALUES ($1,$2, $3) RETURNING commlistid';
      const values = [creatorid, listname, listDescription];
      dbPoolInstance.query(query, values, (error, queryResult) => {
        if (error) {
  ...
  let getCommList = (callback) => {
    
    // console.log(Array.isArray(tags));
    
      let query = 'SELECT * FROM commlistdetail'
      dbPoolInstance.query(query, (error, queryResult) => {
        if (error) {
